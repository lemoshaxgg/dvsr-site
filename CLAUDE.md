...
- Деплоить **батчем после завершения логической задачи**, а не после каждой микро-правки: `npx vercel --prod --yes` из `C:\Users\lemos\dvsr-site`. Перед деплоем — один локальный `npx nuxt build` для проверки ошибок.
- `.vercelignore` уже настроен (режет `.output`/`.nuxt`/`node_modules`). Если деплой висит в `Building` — он держит единственный build-слот free-плана: снять `npx vercel rm <url> --yes`.
